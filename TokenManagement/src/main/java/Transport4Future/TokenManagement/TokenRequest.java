...
public class TokenRequest {
	 ...
	 * @param macAddress
	 */
	public TokenRequest(String deviceName, String typeDevice, String driverVersion, String supportEmail,
		...
		if (this.supportEmail.matches(
				"^[\\\\w!#$%&’*+/=?`{|}~^-]+(?:\\\\.[\\\\w!#$%&’*+/=?`{|}~^-]+)*@(?:[a-zA-Z0-9-]+\\\\.)+[a-zA-Z]{2,6}$")) {
			this.supportEmail = supportEmail;
		} else {
			throw new TokenManagementException("Error: supportEmail uses an invalid format.");
		}
	...
	public String toString() {
		return "TokenRequest [deviceName=" + deviceName + ", typeDevice=" + typeDevice + ", driverVersion="
				+ driverVersion + ", supportEmail=" + supportEmail + ", serialNumber=" + serialNumber + ", macAddress="
				+ macAddress + "]";
	}
}
